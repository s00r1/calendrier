...
بعد تعبئة الجدول يدويًا أو عبر زر **Auto**، قم بتحديث الصفحة. ينبغي أن تظهر القيم نفسها من جديد مما يؤكد حفظها في JSONBin. إذا لم يحدث ذلك، تحقق من ضبط `BIN_URL` و `API_KEY` في ملف `calendar.js`.
