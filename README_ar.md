...
- حفظ التوزيعات عبر خدمة JSONBin من خلال المسار `/api/jsonbin-proxy` (عيِّن متغير البيئة `JSONBIN_KEY` على الخادم)
...
للاحتفاظ بالتوزيعات على الإنترنت، شغّل الخادم الوكيل الموجود في `api/jsonbin-proxy.js` مع تعيين متغيّر البيئة `JSONBIN_KEY`:

```bash
JSONBIN_KEY=مفتاحك node api/jsonbin-proxy.js
```
